...
/*
	...
	一、只能手动使用，如果new而不delete，会内存泄露；如果多次delete会有不确定行为；new得了内存，却越界操作
		如果自定义new，便可在申请所需size的内存后额外神奇一块作为签名，并写入特定的值，在自定义delete检查签名是否正确
		对签名出错的指针进行日志
	二、编译器所提供的new和delete主要用于一般目的，它适用与长时间稳定运行的程序，也使用于瞬间完成的程序，它能胜任各种需求
		大块内存、小块内存、同时大小快内存。它都能适度地好，却没有最佳表现。如果程序申请内存地情况只有很少地几种
		那定制的new和delete会有更好地性能，有时能快数倍

	三、统计数据。对于程序申请的内存，size如何分布？使用时长是多久？分配和归还是倾向于FIFO、LIFO，还是随机顺序？
